...
// 依赖 RLS：user_data 表的行级安全策略确保每用户只能访问自己的数据
...
export async function pullAllFromCloud(localGet, localSet) {
    ...
    try {
        ...
        console.log(`[supabase] pulled ${(data || []).length} items, merged ${merged}`);
        return merged;
    } catch (err) {
...
export async function forcePullFromCloud(localSet) {
    ...
    try {
        ...
        console.log(`[supabase] force pulled ${(data || []).length} items, overwritten ${pulledCount} local items`);
        notifySyncStatus({ syncing: false, pending: 0, lastSync: Date.now() });
        return pulledCount;
    } catch (err) {
...
function isLocalEmptyOrDefault(key, data) {
